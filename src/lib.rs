...
pub async fn run() -> Result<Vec<String>, MusketError> {
    ...
    match cli.cmd {
        Command::Init { force } => {
            handle_init(force, &mut success_messages)?;
        }
        Command::Fire {
            url,
            destination,
            ...
            handle_fire(url, destination, tags, commentary, &mut success_messages).await?;
        }
    }
    Ok(success_messages)
}

fn handle_init(force: Option<bool>, success_messages: &mut Vec<String>) -> Result<(), MusketError> {
...
async fn handle_fire(
    ...
    success_messages: &mut Vec<String>,
) -> Result<(), MusketError> {
